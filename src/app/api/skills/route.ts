...
export async function GET() {
  ...
  if (user) {
    ...
    if (progress) {
      userProgress = Object.fromEntries(progress.map(p => [p.day_id, p.status]));
    }
  }
  ...
  const nodes = (nodesResult.data || []).map(node => {
    ...
    const completedCount = dayIds.filter(id => userProgress[id] === 'completed').length;
    const inProgressCount = dayIds.filter(id => userProgress[id] === 'in_progress').length;
    const availableCount = dayIds.filter(id => userProgress[id] === 'available').length;
