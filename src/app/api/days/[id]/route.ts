...
export async function GET(
  ...
  if (dayResult.error) return NextResponse.json({ error: 'Day not found' }, { status: 404 });
  ...
  if (user) {
    const [progressResult, notesResult] = await Promise.all([
      supabase.from('day_progress').select('*').eq('user_id', user.id).eq('day_id', dayId).single(),
      supabase.from('notes').select('*').eq('user_id', user.id).eq('day_id', dayId).single(),
    ]);
